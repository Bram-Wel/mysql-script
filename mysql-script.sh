...
while true;do
  read -rp "Continue installing mysql version 5.7.* ?? " yn
  case $yn in
    [Yy]* ) yes | sudo apt install -f mysql-client=5.7* mysql-community-server=5.7* mysql-server=5.7*; break;;
    [Nn]* ) echo "Aborting installation!!"; echo "No Clean up done, update script for that."; exit;;
    * ) echo "Please answer Yes or No";;
  esac
done
echo "Do you want to run mysql_secure_installation script? "
